...
/**
 * Pricing rules:
 * - If WASH requested and student has quota & useQuota==true -> wash free (quota consumed at approval).
...
public class PricingService {
    ...
    public BigDecimal computeTotalEstimate(Student student, Set<ServiceType> services, boolean useQuota, Double weightKg, List<OrderItem> itemsToIron) {
        BigDecimal total = BigDecimal.ZERO;

        // Wash
        if (services != null && services.contains(ServiceType.WASH)) {
            if (useQuota && student != null && student.getQuotaRemaining() > 0) {
                // free under quota (default). No addition
            } else {
